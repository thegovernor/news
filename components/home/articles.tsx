...
export function Articles({ articles: initialArticles, title = "مقالات" }: ArticlesProps) {
  ...
  useEffect(() => {
    if (!initialArticles) {
      const fetchArticles = async () => {
        try {
          const data = await client.fetch<Article[]>(ARTICLES_QUERY)
          setArticles(data)
        } catch (error) {
        ...
        <div className="container mx-auto px-4 max-w-7xl">
          <div className="flex items-center gap-3 mb-6 md:mb-8">
            <div className="p-2 bg-primary/10 rounded-lg">
              <Newspaper className="w-5 h-5 md:w-6 md:h-6 text-primary animate-pulse" />
            </div>
